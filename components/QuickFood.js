...
const QuickFood = () => {
  ...
  return (
    <View style = {{margin : 8}}>
      <Text style = {{fontSize : 20 , marginHorizontal : 10 , fontWeight : 'bold'}}>Get it Quickly</Text>
            ...
            data.map((item , index) => (
                <Pressable style ={{margin : 10}} key={index}>
                    <ImageBackground imageStyle={{borderRadius : 6}} style ={{aspectRatio : 5/6 , height : 170}} source={{uri : item.image}}>
                        <Text style={{position : 'absolute' , bottom : 10 , left : 10 , fontSize : 27 , fontWeight : '900' , color : 'white'}}>{item.offer}OFF</Text>
                    </ImageBackground>
                    <Text style={{marginTop : 10 , fontSize : 16 , textAlign : 'center' , fontWeight : '500'}}>{item.name}</Text>
